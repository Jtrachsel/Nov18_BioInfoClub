...
Once you have loaded all the required modules, and cloned the repository to your own folder we can proceed to run this script.
...
In the folder we downloaded from github, there should be a file called 'SLURM.bash'.  
...
Enter the following command:  
```{r, engine='bash', eval=FALSE}
scp -r user.name@scinet-login.bioteam.net:/projects/fsepru/YOURFOLDER/Nov18_BioInfoClub ./from_scinet
...
```{r}
setwd("~/biclub/finalversion/Nov18_BioInfoClub/")
library(ggtree)
library(ape)
